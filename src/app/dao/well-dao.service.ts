...
export class WellDaoService {
  ...
  //         wells.push(new WellClass(well.id,well.permitNumber,well.wellName,well.wellNumber,well.county,
  ...
  public getWell(id: string):WellClass{
    let well : WellClass = this.wells[0];
  ...
  public insertProduction(id: string, type: string, quantity: number, dateProduced: any){
    ...
    if(indexPos >= 0){
      // this.wells[indexPos].getProduction().push(new ProductionClass(type,quantity,dateProduced))
    }
  }
  ...
  public deleteProduction(id: string, position: number) : void{
    const well: WellClass = this.wells[this.getWellIndex(id)];
